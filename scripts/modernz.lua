...
local user_opts = {
	...
	hidetimeout = 500, -- time (in ms) before OSC hides if no mouse movement
	...
	mouse_seek_pause = false, -- pause video while seeking with mouse move (on button hold)
...
local function set_osc_styles()
	...
	osc_styles = {
		...
		title = "{\\blur0\\bord1\\1c&H"
			...
			.. user_opts.font
			.. "}",
		chapter_title = "{\\blur0\\bord1\\1c&H"
			...
			if element.name ~= "persistent_seekbar" then
				...
				if element.slider and element.slider.tooltipF ~= nil and element.enabled then
					...
					if mouse_hit(element) or force_seek_tooltip then
						...
						local thumbnail_radius = user_opts.thumbnail_box_radius > 0 and user_opts.thumbnail_box_radius
							or 0
...
layouts["modern"] = function()
	...
	local title_offset = (no_chapter or not chapter_index) and user_opts.title_offset
	...
	if title_and_chapter_h_with_offset == 0 then
		...
		title_and_chapter_h_with_offset = math.max(
	...
	local title_y = (no_chapter or not chapter_index) and (user_opts.osc_height + title_offset)
	...
	if user_opts.show_chapter_title then
		...
		lo.geometry = {
			...
			an = 1,
			w = osc_geo.w - time_codes_width - 50,
	...
	if narrow_win then
		...
		time_codes_y = (no_chapter or not chapter_index) and (time_codes_y + user_opts.title_offset)
			or (time_codes_y + user_opts.chapter_title_offset)
	end
	...
	if fullscreen_button then
		right_side_button("fullscreen", 550)
	end
	if info_button then
		right_side_button("info", 650)
	end
	if ontop_button then
		right_side_button("ontop", 750)
	end
	if screenshot_button then
		right_side_button("screenshot", 850)
	end
	if loop_button then
		right_side_button("file_loop", 950)
	end
	if shuffle_button then
		right_side_button("shuffle", 1050)
	end
	if speed_button then
		right_side_button("speed", 1150, osc_styles.speed, 42)
	end
	if download_button then
		right_side_button("download", 1150)
	end
...
layouts["modern-compact"] = function()
	...
	lo = add_layout("time_codes")
	lo.geometry =
		{ x = osc_geo.w - 25, y = refY - time_codes_y, an = 3, w = time_codes_width, h = user_opts.time_font_size }
	lo.layer = 48
	lo.alpha[3] = 0
	lo.style = osc_styles.time
	...
	if elements["cache_info"].visible then
		...
		lo.geometry = {
			...
			y = refY - (user_opts.osc_height / 2),
			an = 6,
			w = (user_opts.cache_info_speed and 70 or 45),
			h = 24,
		}
		lo.alpha[3] = 0
...
local function osc_init()
	...
	ne.tooltipF = function()
		return (user_opts.tooltip_hints and cache_enabled()) and locale.cache or ""
	end
	ne.eventresponder["mbtn_left_up"] = function()
